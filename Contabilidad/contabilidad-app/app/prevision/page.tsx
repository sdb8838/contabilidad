...
import { TIPOS_PREVISION, Operacion, Partida, CENTROS_GESTORES_INICIALES, PARTIDAS_INICIALES, EJERCICIO_DEFAULT, EJERCICIOS } from "@/lib/data";
...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/data").then(r => r.json()).then(d => {
      setData((d.records || []).filter((op: Operacion) => TIPOS_PREVISION.includes(op.tipo_operacion)));
  ...
  const save = async (updated: Operacion[]) => {
    ...
    setData(updatedData);
    // Mantener operaciones no-PREVISION
    const newAll = [
      ...allData.filter(op => op.tipo_operacion !== "Previsión"),
      ...normalized.map(op => ({ ...op, tipo_operacion: "Previsión" }))
    ];
    setAllData(newAll);
  ...
  const exportData = () => {
    exportToXlsx(filtered.map(op => ({ Ejercicio: op.ejercicio, CentroGestor: op.centro_gestor, Partida: op.partida_economico, Importe: op.importe_total, Descripcion: op.descripcion || "" })), `prevision_${cg}_${ej}`, "Prevision");
  };
  ...
  const importFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const onComplete = (rows: Record<string, unknown>[]) => {
      const newOps = rows.map(row => ({
        ...
        tipo_operacion: "Previsión", importe_total: Math.abs(Number(row.importe_total || row.Importe || 0)),
      ...
      // Usar todas las operaciones Previsión (no solo del centro actual)
      const prevOps = allData.filter(op => op.tipo_operacion === "Previsión");
      save([...prevOps, ...newOps]);
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold mb-6 text-murcia-green">Previsión</h1>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
          <div className="flex justify-between items-center"><span className="text-lg font-semibold text-yellow-800">Total Previsión</span>
            <span className="text-2xl font-bold text-yellow-700">{total.toLocaleString("es-ES", { style: "currency", currency: "EUR" })}</span>
          </div>
        </div>
        <div className="card">
          <EditableTable data={filtered} columns={columns} onSave={save} tipoOperacion="Previsión" partidas={filteredPartidas} centrosGestores={CENTROS_GESTORES_INICIALES} ejercicioDefault={ej} centroGestorDefault={cg} onExport={exportData} onImport={() => fileRef.current?.click()} />
